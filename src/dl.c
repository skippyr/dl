...
static struct Credential *findCredential(uid_t id, int isUser)
{
	...
	for (size_t offset = 0; offset < credentialsAllocator->use; ++offset) {
		...
		if ((credential = (struct Credential *)credentialsAllocator->buffer + offset)
				->id == id) {
			return credential;
		}
	}
...
format_l:
	...
	*(short *)(formatBuffer + formatOffset) = *(short *)"B";
	char *buffer =
		allocateArenaMemory(entriesDataAllocator_g, (*length = formatOffset + 1) + 1);
...
static void readDirectory(const char *directoryPath)
{
	...
	allocateArenaAllocator(&userCredentialsAllocator_g, "userCredentialsAllocator_g",
						   sizeof(struct Credential), 20);
	allocateArenaAllocator(&groupCredentialsAllocator_g, "groupCredentialsAllocator_g",
						   sizeof(struct Credential), 20);
	...
	for (long totalEntries; (totalEntries = syscall(SYS_getdents64, directoryStream,
													buffer_g, BUFFER_SIZE)) > 0;) {
	...
	qsort(entriesAllocator_g->buffer, entriesAllocator_g->use, sizeof(struct Entry),
	...
	temporaryDataAllocator_g->use = 0;
	printf("%*s %-*s %-*s %-*s %*s %-*s Name\n", noColumnLength, "No.", groupColumnLength,
		   "Group", userColumnLength, "User", entriesAllocator_g->use ? 17 : 13,
		   "Modified Date", sizeColumnLength, "Size", entriesAllocator_g->use ? 13 : 11,
		   "Permissions");
	...
	for (size_t offset = 0; offset < entriesAllocator_g->use; ++offset) {
		...
		tdk_set256Color(tdk_Color_Magenta, tdk_Layer_Foreground);
		printf(" %03o ", entry->mode & (S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP |
...
int main(int totalArguments, const char **arguments)
{
	...
	for (int offset = 1; offset < totalArguments; ++offset) {
		...
		PARSE_OPTION("version",
					 printf("dl %s (compiled for Linux %s)\n", VERSION, ARCHICTECTURE));
