...
static void allocateArenaAllocator(struct ArenaAllocator **allocator,
                                   const char *name, size_t unit,
                                   size_t capacity) {
  if (*allocator) {
    return;
  }
...
static void *allocateArenaMemory(struct ArenaAllocator *allocator, size_t use) {
  ...
  writeLog(LogType_Error, "can not allocate %zuB of memory on arena \"%s\".",
           allocator->name);
  return NULL;
}
...
static struct Credential *findCredential(uid_t id, int isUser) {
  ...
  for (size_t offset = 0; offset < credentialsAllocator->use; ++offset) {
    ...
    if ((credential =
             (struct Credential *)credentialsAllocator->buffer + offset)
            ->id == id) {
      return credential;
    }
  }
  ...
  credential->name =
      allocateArenaMemory(credentialsDataAllocator, credential->nameLength + 1);
  ...
  for (int offset = 0; offset < 4; ++offset) {
    if (status->st_size >= multiplierValues[offset]) {
      ...
      sprintf(numberBuffer,
              ((temporary = size - (int)size) >= 0 && temporary < 0.1) ||
                      temporary >= 0.95
                  ? "%.0f"
                  : "%.1f",
              size);
...
format_l:
  ...
  int formatOffset = 0;
  for (int numberOffset = 0; numberBuffer[numberOffset];
       (void)(++numberOffset && ++formatOffset)) {
  ...
  *(short *)(formatBuffer + formatOffset) = *(short *)"B";
  char *buffer = allocateArenaMemory(entriesDataAllocator_g,
                                     (*length = formatOffset + 1) + 1);
...
static void readDirectory(const char *directoryPath) {
  ...
  if (stat(directoryPath, &status)) {
    ...
    writeLog(LogType_Warning, "the entry \"%s\" is not a directory.",
             directoryPath);
    return;
  } else if ((directoryStream = open(directoryPath, O_RDONLY)) < 0) {
    writeLog(LogType_Warning, "can not open the directory \"%s\".",
             directoryPath);
    return;
  }
  if (!buffer_g) {
  ...
  allocateArenaAllocator(&userCredentialsAllocator_g,
                         "userCredentialsAllocator_g",
                         sizeof(struct Credential), 20);
  allocateArenaAllocator(&groupCredentialsAllocator_g,
                         "groupCredentialsAllocator_g",
                         sizeof(struct Credential), 20);
  allocateArenaAllocator(&userCredentialsDataAllocator_g,
                         "userCredentialsDataAllocator_g", sizeof(char), 320);
  allocateArenaAllocator(&groupCredentialsDataAllocator_g,
                         "groupCredentialsDataAllocator_g", sizeof(char), 320);
  allocateArenaAllocator(&entriesAllocator_g, "entriesAllocator_g",
                         sizeof(struct Entry), 30000);
  allocateArenaAllocator(&entriesDataAllocator_g, "entriesDataAllocator_g",
                         sizeof(char), 2097152);
  allocateArenaAllocator(&temporaryDataAllocator_g, "temporaryDataAllocator_g",
                         sizeof(char), 600);
  ...
  for (long totalEntries;
       (totalEntries = syscall(SYS_getdents64, directoryStream, buffer_g,
                               BUFFER_SIZE)) > 0;) {
    struct linux_dirent64 *linuxEntry;
    for (long offset = 0; offset < totalEntries;
         offset += linuxEntry->d_reclen) {
      linuxEntry = (struct linux_dirent64 *)(buffer_g + offset);
      if (linuxEntry->d_name[0] == '.' &&
          (!linuxEntry->d_name[1] ||
           (linuxEntry->d_name[1] == '.' && !linuxEntry->d_name[2]))) {
        continue;
      }
      ...
      entry->name =
          allocateArenaMemory(entriesDataAllocator_g, entryNameLength + 1);
      ...
      if (linuxEntry->d_type == DT_LNK) {
        ...
        entry->link =
            allocateArenaMemory(entriesDataAllocator_g, linkLength + 1);
  ...
  int totalDigitsInMaximumNo =
      countDigits(entriesAllocator_g->use) + (entriesAllocator_g->use > 1000);
  ...
  printf("%*s %-*s %-*s %-*s %*s %-*s Name\n", noColumnLength, "No.",
         groupColumnLength, "Group", userColumnLength, "User",
         entriesAllocator_g->use ? 17 : 13, "Modified Date", sizeColumnLength,
         "Size", entriesAllocator_g->use ? 13 : 11, "Permissions");
  writeLines(7, noColumnLength, groupColumnLength, userColumnLength,
             entriesAllocator_g->use ? 17 : 13, sizeColumnLength,
             entriesAllocator_g->use ? 13 : 11, nameColumnLength);
  if (!entriesAllocator_g->use) {
    tdk_set256Color(tdk_Color_LightBlack, tdk_Layer_Foreground);
    printf("%*s\n",
           14 + noColumnLength + groupColumnLength + userColumnLength +
               sizeColumnLength + nameColumnLength,
           "DIRECTORY IS EMPTY");
    tdk_set256Color(tdk_Color_Default, tdk_Layer_Foreground);
  }
...
static void writeLog(int type, const char *format, ...) {
  tdk_set256Color(type == LogType_Warning ? tdk_Color_Yellow : tdk_Color_Red,
...
int main(int totalArguments, const char **arguments) {
  ...
  for (int offset = 1; offset < totalArguments; ++offset) {
    ...
    if (arguments[offset][0] == '-' &&
        (arguments[offset][1] == '-' || !arguments[offset][2])) {
      writeLog(LogType_Error, "the option \"%s\" is unrecognized.",
               arguments[offset]);
    }
  }
  for (int offset = 1; offset < totalArguments; ++offset) {
